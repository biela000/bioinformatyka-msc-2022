...
export default function useWindowOptions(chain: Codon[]) {
	...
	const { windowOptions, biggestPossibleWindow } = useMemo(() => {
		...
		const biggestPossibleWindow =
			allWindows.find(window => chain.length >= window * 2) ?? 3;
		const biggestPossibleWindowIndex = allWindows.indexOf(
			biggestPossibleWindow,
		);
		const windows = allWindows.slice(biggestPossibleWindowIndex);
		const windowOptions = windows.map((window, index) => (
