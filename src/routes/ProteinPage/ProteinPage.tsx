...
function ProteinPage() {
	...
	const proteinString = protein.aminoAcidChain.map(aminoAcid => aminoAcid.aminoAcidLetter).join('');

	return (
		<div className={classes.proteinPage} data-testid="ProteinPageComponent">
			<AminoAcidChain chain={protein.aminoAcidChain} />
			<ProteinStructure proteinString={proteinString} mass={protein.mass} />
			<Link to={`/result/${shiftId + 1}/protein/${proteinId + 1}/details`} className={classes['protein-link']}>Details</Link>
		</div>
	);
}
